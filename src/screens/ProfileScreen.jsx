...
export default function ProfileScreen() {
  ...
  const handleAvatarUpload = async (e) => {
    ...
    reader.onload = async () => {
      ...
      // Сохраняем в Firestore по storedUser.name
      try {
        await updateDoc(doc(db, "users", storedUser.name), { avatar: reader.result });
      } catch (err) {
        // обработка ошибки
      }
    };
  ...
  const handleAvatarDelete = async () => {
    ...
    // Удаляем аватар из Firestore по storedUser.name
    try {
      await updateDoc(doc(db, "users", storedUser.name), { avatar: null });
    } catch (err) {
      // обработка ошибки
    }
  };
